...
categoriesToScan.forEach((category) => {
  ...
  try {
    if (fs.existsSync(categoryDirectory)) {
        ...
        if (/\.(png|jpe?g|gif|webp)$/i.test(file)) {
          ...
          if (fileNameWithoutExtension.startsWith("Futur-O-Mat__")) {
            ...
            if (category === "head" || category === "bodytype") {
              ...
              const skinToneMatch = fileNameWithoutExtension.match(
                /_(Hell|Braun|Dunkel)_(.+)$/,
              );
              if (skinToneMatch) {
                const skinTone = skinToneMatch[1].toLowerCase();
                const shape = skinToneMatch[2]
                  .toLowerCase()
                  .replace(/[^a-z0-9]/g, "_");
                ...
                partId = `${category}_${descriptivePart}`
                  .toLowerCase()
                  .replace(/[^a-z0-9]/g, "_");
              }
            } else if (category === "hair") {
              // Pattern: Futur-O-Mat__black_0000_Offen-lang-Kraus
              const hairColorMatch = fileNameWithoutExtension.match(
                /__(black|blonde|brunette|red|white)_[^_]*_(.+)$/,
              );
              if (hairColorMatch) {
                const hairColor = hairColorMatch[1];
                const style = hairColorMatch[2]
                  .toLowerCase()
                  .replace(/[^a-z0-9]/g, "_");
            ...
            partId = `${category}_${fileNameWithoutExtension}`
              .toLowerCase()
              .replace(/[^a-z0-9]/g, "_");
          }
      ...
      console.warn(
        `Warning: Directory not found for category '${category}': ${categoryDirectory}`,
      );
    }
  } catch (err) {
    console.error(
      `Error reading directory for category '${category}' (${categoryDirectory}):`,
      err,
    );
  }
});
...
try {
  fs.writeFileSync(
    outputManifestPath,
    JSON.stringify(generatedAvatarParts, null, 2),
  );
  console.log(
    `Avatar parts manifest successfully generated at: ${outputManifestPath}`,
  );
  if (generatedAvatarParts.length === 0) {
    console.warn(
      "Warning: The generated manifest is empty. Check your asset paths and categories.",
    );
  }
} catch (err) {
