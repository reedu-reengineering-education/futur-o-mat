  ...
  if (state.availableParts && state.availableParts.length > 0) {
    ...
    if (encodedAvatarState) {
      ...
      if (!decodedSuccessfully) {
        console.warn(
          "Failed to decode avatar state from URL, generating random avatar instead.",
        );
    ...
    canvas.toBlob(
      async (blob) => {
        ...
        formData.append(
          "image",
          blob,
          `futur-o-mat-${stepType}-${Date.now()}.png`,
        );
        ...
        try {
          ...
          console.log(
            "Response received from upload_image.php. Status:",
            response.status,
            "OK:",
            response.ok,
          );
...
function decodeAvatarState(encodedState) {
  try {
    ...
    if (
      decoded.selectedParts &&
      decoded.selectedItems &&
      decoded.currentSkinTone &&
      decoded.currentHairColor !== undefined &&
      decoded.currentBrustAnsatz !== undefined
    ) {
...
function showSharingModal(imageUrl, stepType = "avatar") {
  ...
  const description =
    "Erstellt mit dem Futur-O-Mat - Mach dir die Zukunft, wie sie dir gefällt!";
...
async function loadShareButtons(imageUrl, title, description, pageUrl) {
  // pageUrl is now sharePageUrl
  try {
...
function copyToClipboard(imageUrl, title) {
  ...
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard
      ...
      .then(() => {
        alert(
          "Link wurde kopiert! Du kannst ihn jetzt in Signal oder andere Apps einfügen.",
        );
      })
      .catch((err) => {
